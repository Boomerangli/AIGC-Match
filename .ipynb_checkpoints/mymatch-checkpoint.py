...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            pred_u_w_fpw = model(img_u_w, weak=True,k=kr).detach()
            conf_u_w_fpw = pred_u_w_fpw.softmax(dim=1).max(dim=1)[0]
            mask_u_w_fpw = pred_u_w_fpw.argmax(dim=1)
            
            preds, preds_fp = model(torch.cat((img_x, img_u_w)), False, False, False, False, True, kr).chunk(2)
            #import ipdb
            #ipdb.set_trace()
            ...
            '''
            loss_u_w_fp = criterion_dice(pred_u_w_fp.softmax(dim=1), mask_u_w_fpw.unsqueeze(1).float(),
                                         ignore=(conf_u_w_fpw < cfg['conf_thresh']).float())
